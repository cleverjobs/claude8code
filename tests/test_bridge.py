...
class TestModelMapping:
    ...
    def test_direct_model_ids(self) -> None:
    ...
    def test_all_models_have_valid_targets(self) -> None:
...
class TestSDKMessageMode:
    ...
    def test_get_mode_invalid_falls_back(self) -> None:
    ...
    def test_get_mode_none_uses_default(self) -> None:
...
class TestXMLFormatting:
    ...
    def test_format_tool_use_as_xml(self) -> None:
    ...
    def test_format_tool_use_complex_input(self) -> None:
        """Test tool use with complex nested input."""
        result = format_tool_use_as_xml(
            "search", {"query": "test", "options": {"limit": 10, "sort": "date"}}
        )
        assert '<tool_use name="search">' in result
        assert '"options"' in result
        assert "</tool_use>" in result

    def test_format_tool_result_as_xml(self) -> None:
...
class TestApplyMessageMode:
    ...
    def test_forward_mode_preserves_blocks(self) -> None:
        """Test forward mode returns blocks unchanged."""
        blocks = [
            TextBlock(text="Hello"),
            ToolUseResponseBlock(id="tool_1", name="test", input={}),
        ]
        result = apply_message_mode(blocks, SDKMessageMode.FORWARD)  # type: ignore[arg-type]
        assert len(result) == 2
        assert result[0].type == "text"
        assert result[1].type == "tool_use"

    def test_ignore_mode_filters_tool_blocks(self) -> None:
    ...
    def test_formatted_mode_converts_tools(self) -> None:
        ...
        texts = [b.text for b in result]  # type: ignore[union-attr]
